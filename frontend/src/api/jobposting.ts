...
export const jobpostingService = {
  async getById(jobpostingId: string | number): Promise<Jobposting> {
  ...
  async update(jobpostingId: string | number, request: JobpostingUpdateRequest): Promise<Jobposting> {
    return api.put(`/api/v1/jobpostings/${jobpostingId}`, request)
  },

  async delete(jobpostingId: string | number): Promise<void> {
...
export const readService = {
  // 단일 채용공고 조회 (통계 포함)
  async getById(jobpostingId: string | number): Promise<JobpostingReadResponse> {
...
export const commentService = {
  ...
  async getList(jobpostingId: string | number, page: number, pageSize: number): Promise<CommentPageResponse> {
...
export const viewService = {
  // 조회수 증가 (인증 필요)
  async increaseViewCount(jobpostingId: string | number): Promise<number> {
    return api.post(`/api/v1/jobposting-views/jobpostings/${jobpostingId}`)
  },

  // 조회수 조회 (인증 불필요)
  async getViewCount(jobpostingId: string | number): Promise<number> {
...
export const likeService = {
  // 좋아요 상태 조회 (인증 필요)
  async getLikeStatus(jobpostingId: string | number): Promise<LikeResponse> {
    return api.get(`/api/v1/jobposting-likes/jobpostings/${jobpostingId}`)
  },

  // 좋아요 수 조회 (인증 불필요)
  async getLikeCount(jobpostingId: string | number): Promise<number> {
    return api.get(`/api/v1/jobposting-likes/jobpostings/${jobpostingId}/count`)
  },

  // 좋아요 (인증 필요)
  async like(jobpostingId: string | number): Promise<void> {
    return api.post(`/api/v1/jobposting-likes/jobpostings/${jobpostingId}`)
  },

  // 좋아요 취소 (인증 필요)
  async unlike(jobpostingId: string | number): Promise<void> {
