...
// 댓글 페이지 로드 (UI는 1-based, 백엔드는 0-based)
function loadComments(page: number) {
  if (!jobposting.value) return
  jobpostingStore.fetchComments(jobposting.value.jobpostingId, page - 1)
}

// 재시도
function retry() {
  const jobpostingId = route.params.id as string
...
onMounted(async () => {
  ...
  try {
    ...
    await jobpostingStore.fetchComments(jobpostingId, 0)
