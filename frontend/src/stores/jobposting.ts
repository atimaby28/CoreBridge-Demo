...
export const useJobpostingStore = defineStore('jobposting', () => {
  ...
  async function fetchJobposting(jobpostingId: string | number, isLoggedIn: boolean = false): Promise<JobpostingDetail> {
    loading.value = true
    error.value = null
  ...
  // 채용공고 수정
  async function updateJobposting(jobpostingId: string | number, request: JobpostingUpdateRequest): Promise<Jobposting> {
    loading.value = true
    error.value = null
  ...
  // 채용공고 삭제
  async function deleteJobposting(jobpostingId: string | number): Promise<void> {
    loading.value = true
    error.value = null
  ...
  // 좋아요 토글
  async function toggleLike(jobpostingId: string | number): Promise<void> {
  ...
  // 댓글 목록 조회
  async function fetchComments(jobpostingId: string | number, page?: number): Promise<void> {
    loading.value = true
    error.value = null
  ...
  async function createComment(request: CommentCreateRequest): Promise<Comment> {
    ...
    try {
      ...
      // 댓글 목록 갱신
      await fetchComments(request.jobpostingId, 0)
      // 댓글 수 증가
  ...
  // 댓글 삭제
  async function deleteComment(commentId: number, jobpostingId: string | number): Promise<void> {
    loading.value = true
    error.value = null
