...
export const useApplyStore = defineStore('apply', () => {
   ...
   * 공고별 지원자 목록 조회
   */
  async function fetchAppliesByJobposting(jobpostingId: string | number): Promise<void> {
    loading.value = true
    error.value = null
    try {
   ...
   * 공고별 특정 단계 지원자 목록 조회
   */
  async function fetchAppliesByStep(jobpostingId: string | number, step: ProcessStep): Promise<void> {
    loading.value = true
    error.value = null
    try {
